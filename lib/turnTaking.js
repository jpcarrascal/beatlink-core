...
class TurnTakingManager {
    constructor({ io, sessions, logger, registry }) {
        ...
        this.ctx = null; // plugin context, assigned by the runtime after build
        this.timers = new Map(); // `${sessionName}:${slot}` -> timeout id
    }

    runActivateHooks(session, info) {
        this.registry.activateHooks.forEach(hook => hook(session, info, this.ctx));
    }

    runReleaseHooks(session, info) {
        this.registry.releaseHooks.forEach(hook => hook(session, info, this.ctx));
    }
    ...
    leave(sessionName, socketID, initials) {
        ...
        if (slot >= 0) {
            ...
            this.runReleaseHooks(session, { slot, socketID, initials, reason: 'disconnect' });
    ...
    activate(sessionName, session, socketID, slot, initials) {
        ...
        this.runActivateHooks(session, { slot, socketID, initials });
        this.emitQueueUpdates(sessionName, session);
    ...
    evict(sessionName, session, socketID, reason) {
        ...
        session.touch();
        this.io.to(sessionName).emit('participant-left', { slot, initials, socketID });
        this.runReleaseHooks(session, { slot, socketID, initials, reason: 'evicted' });
    ...
    forceAllToQueue(sessionName, reason = 'Session is temporarily suspended by the host.') {
        ...
        session.participants.snapshot().forEach(({ slot, socketID, initials }) => {
            ...
            session.participants.release(socketID);
            this.io.to(sessionName).emit('participant-left', { slot, initials, socketID });
            this.runReleaseHooks(session, { slot, socketID, initials, reason: 'forced' });
